...
namespace cookie {
	...
	private:
		...
		static std::shared_ptr <Shader> uberShader;
	...
	protected:
		...
		[[nodiscard]] virtual std::unique_ptr <cookie::PlatformSpecificData>
		createPlatformSpecificContainerImpl() const = 0;
	...
	public:
		...
		static std::shared_ptr <Shader> getUberShader();
		...
		static std::unique_ptr <cookie::BoundsRenderer> provideBoundsRenderer();
		static std::unique_ptr <cookie::DebugRenderer> provideDebugRenderer();
		...
		static std::unique_ptr <cookie::SceneSectorManager>
		provideSceneSectorManager(float sectorSize, const Bounds &bounds);
	};
}
