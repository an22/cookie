...
namespace cookie {
	...
	void Batch::saveToGPU() {
		...
		saveToGPUInternal(meshData, matrices);
	}

	void Batch::draw(const DrawUtils &drawUtils) {
	...
	void Batch::combineMeshData(std::vector<Vertex> &vertices, std::vector<uint32_t> &indices, std::vector<glm::mat4> &matrices) {
	...
	void Batch::calculateVertexAndIndexSize(uint32_t &vertexCount, uint32_t &indexCount) {
		vertexCount = 0;
		indexCount = 0;
		for (auto &sceneObject: sceneObjects) {
			const auto &mesh = sceneObject->getComponent<MeshComponent>();
